					...
					struct timer_info *info)
{
	...
	pr_debug("%s (ch:%d, mux=%d, scl=%d, rate=%ld, %s)\n",
		__func__, info->channel, tmux, tscl, tout, from_tclk?"TCLK":"PCLK");
}
...
static void __init timer_of_init_dt(struct device_node *node)
{
	...
	if (!dev)
		panic("Failed to allocate clock source/event device!\n");
...
CLOCKSOURCE_OF_DECLARE(nexell, "nexell,nxp-timer", timer_of_init_dt);
