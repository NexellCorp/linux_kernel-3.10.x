...
static void pll_set_rate(int PLL, int P, int M, int S)
{
	 ...
	 */
	reg->PLLSETREG[PLL] =((1UL << 29)			| // power down
...
static inline unsigned int pll_div(int dvo)
{
	...
	return  ((((val>>DVO3)&0x3F)+1)<<24) | ((((val>>DVO2)&0x3F)+1)<<16) |
			((((val>>DVO1)&0x3F)+1)<<8)  | ((((val>>DVO0)&0x3F)+1)<<0);
}

#define	PLLN_RATE(n)		(pll_rate(n, ref_clk))	/* 0~ 3 */
#define	CPU_FCLK_RATE(n)	(pll_rate(pll_dvo(n), ref_clk) / ((pll_div(n)>> 0)&0x3F))
#define	CPU_HCLK_RATE(n)	(pll_rate(pll_dvo(n), ref_clk) /		\
				  			((pll_div(n)>> 0)&0x3F) / ((pll_div(n)>> 8)&0x3F))
#define	MEM_FCLK_RATE()		(pll_rate(pll_dvo(DIV_MEM), ref_clk) / 	\
							((pll_div(DIV_MEM)>> 0)&0x3F) / ((pll_div(DIV_MEM)>> 8)&0x3F))
#define	MEM_DCLK_RATE()		(pll_rate(pll_dvo(DIV_MEM), ref_clk) /		\
				  			((pll_div(DIV_MEM)>> 0)&0x3F))
#define	MEM_BCLK_RATE()		(pll_rate(pll_dvo(DIV_MEM), ref_clk) /		\
			  				((pll_div(DIV_MEM)>> 0)&0x3F) /						\
			  				((pll_div(DIV_MEM)>> 8)&0x3F) /						\
			  				((pll_div(DIV_MEM)>>16)&0x3F))
#define	MEM_PCLK_RATE()		(pll_rate(pll_dvo(DIV_MEM), ref_clk) /		\
			  				((pll_div(DIV_MEM)>> 0)&0x3F) /						\
			  				((pll_div(DIV_MEM)>> 8)&0x3F) /						\
			  				((pll_div(DIV_MEM)>>16)&0x3F) /						\
			  				((pll_div(DIV_MEM)>>24)&0x3F))
#define	BUS_BCLK_RATE()		(pll_rate(pll_dvo(DIV_BUS), ref_clk) /		\
					  		((pll_div(DIV_BUS)>> 0)&0x3F))
#define	BUS_PCLK_RATE()		(pll_rate(pll_dvo(DIV_BUS), ref_clk) /		\
			  				((pll_div(DIV_BUS)>> 0)&0x3F) / ((pll_div(DIV_BUS)>> 8)&0x3F))
#define	G3D_BCLK_RATE()		(pll_rate(pll_dvo(DIV_G3D), ref_clk) /		\
			  				((pll_div(DIV_G3D)>> 0)&0x3F))
#define	VPU_BCLK_RATE()		(pll_rate(pll_dvo(DIV_VPU), ref_clk) /		\
			  				((pll_div(DIV_VPU)>> 0)&0x3F))
#define	VPU_PCLK_RATE()		(pll_rate(pll_dvo(DIV_VPU), ref_clk) /		\
			  				((pll_div(DIV_VPU)>> 0)&0x3F)	/					\
			  				((pll_div(DIV_VPU)>> 8)&0x3F))
#define	DIS_BCLK_RATE()		(pll_rate(pll_dvo(DIV_DISP), ref_clk) /		\
			  				((pll_div(DIV_DISP)>> 0)&0x3F))
#define	DIS_PCLK_RATE()		(pll_rate(pll_dvo(DIV_DISP), ref_clk) /		\
			  				((pll_div(DIV_DISP)>> 0)&0x3F)	/					\
			  				((pll_div(DIV_DISP)>> 8)&0x3F))
#define	HDMI_PCLK_RATE()	(pll_rate(pll_dvo(DIV_HDMI), ref_clk) /		\
			  				((pll_div(DIV_HDMI)>> 0)&0x3F))
#define	CCI_BCLK_RATE()		(pll_rate(pll_dvo(DIV_CCI4), ref_clk) /		\
			  				((pll_div(DIV_CCI4)>> 0)&0x3F))
#define	CCI_PCLK_RATE()		(pll_rate(pll_dvo(DIV_CCI4), ref_clk) /		\
			  				((pll_div(DIV_CCI4)>> 0)&0x3F)	/					\
...
static struct clk_core clk_pll_dev[] = {
	[ID_CPU_PLL0] = { .id = ID_CPU_PLL0, .name = CLK_CPU_PLL0, },
	[ID_CPU_PLL1] = { .id = ID_CPU_PLL1, .name = CLK_CPU_PLL1, },
	[ID_CPU_PLL2] = { .id = ID_CPU_PLL2, .name = CLK_CPU_PLL2, },
	[ID_CPU_PLL3] = { .id = ID_CPU_PLL3, .name = CLK_CPU_PLL3, },
	[ID_CPU_FCLK] = { .id = ID_CPU_FCLK, .name = CLK_CPU_FCLK, .div = DIV_CPUG0},
	[ID_CPU_HCLK] = { .id = ID_CPU_HCLK, .name = CLK_CPU_HCLK, .div = DIV_CPUG0},
	[ID_MEM_FCLK] = { .id = ID_MEM_FCLK, .name = CLK_MEM_FCLK, .div = DIV_MEM  },
	[ID_MEM_DCLK] = { .id = ID_MEM_DCLK, .name = CLK_MEM_DCLK, .div = DIV_MEM  },
	[ID_MEM_BCLK] = { .id = ID_MEM_BCLK, .name = CLK_MEM_BCLK, .div = DIV_MEM  },
	[ID_MEM_PCLK] = { .id = ID_MEM_PCLK, .name = CLK_MEM_PCLK, .div = DIV_MEM  },
	[ID_BUS_BCLK] = { .id = ID_BUS_BCLK, .name = CLK_BUS_BCLK, .div = DIV_BUS  },
	[ID_BUS_PCLK] = { .id = ID_BUS_PCLK, .name = CLK_BUS_PCLK, .div = DIV_BUS  },
	[ID_VPU_BCLK] = { .id = ID_VPU_BCLK, .name = CLK_VPU_BCLK, .div = DIV_VPU  },
	[ID_VPU_PCLK] = { .id = ID_VPU_PCLK, .name = CLK_VPU_PCLK, .div = DIV_VPU  },
	[ID_DIS_BCLK] = { .id = ID_DIS_BCLK, .name = CLK_DIS_BCLK, .div = DIV_DISP },
	[ID_DIS_PCLK] = { .id = ID_DIS_PCLK, .name = CLK_DIS_PCLK, .div = DIV_DISP },
	[ID_CCI_BCLK] = { .id = ID_CCI_BCLK, .name = CLK_CCI_BCLK, .div = DIV_CCI4 },
	[ID_CCI_PCLK] = { .id = ID_CCI_PCLK, .name = CLK_CCI_PCLK, .div = DIV_CCI4 },
	[ID_G3D_BCLK] = { .id = ID_G3D_BCLK, .name = CLK_G3D_BCLK, .div = DIV_G3D  },
};
...
static unsigned long clk_pll_recalc_rate(struct clk_hw *hw, unsigned long rate)
{
	...
	switch (id) {
		case ID_CPU_PLL0: rate = PLLN_RATE(0);    	break;
		case ID_CPU_PLL1: rate = PLLN_RATE(1);    	break;
		case ID_CPU_PLL2: rate = PLLN_RATE(2);    	break;
		case ID_CPU_PLL3: rate = PLLN_RATE(3);    	break;
		case ID_CPU_FCLK: rate = CPU_FCLK_RATE(DIV_CPUG0);  break;
		case ID_CPU_HCLK: rate = CPU_HCLK_RATE(DIV_CPUG0);  break;
		case ID_MEM_FCLK: rate = MEM_FCLK_RATE();  	break;
		case ID_BUS_BCLK: rate = BUS_BCLK_RATE();  	break;
		case ID_BUS_PCLK: rate = BUS_PCLK_RATE();  	break;
		case ID_MEM_DCLK: rate = MEM_DCLK_RATE();  	break;
		case ID_MEM_BCLK: rate = MEM_BCLK_RATE();  	break;
		case ID_MEM_PCLK: rate = MEM_PCLK_RATE();  	break;
		case ID_G3D_BCLK: rate = G3D_BCLK_RATE();	break;
		case ID_VPU_BCLK: rate = VPU_BCLK_RATE(); 	break;
		case ID_VPU_PCLK: rate = VPU_PCLK_RATE(); 	break;
		case ID_DIS_BCLK: rate = DIS_BCLK_RATE(); 	break;
		case ID_DIS_PCLK: rate = DIS_PCLK_RATE(); 	break;
		case ID_CCI_BCLK: rate = CCI_BCLK_RATE(); 	break;
		case ID_CCI_PCLK: rate = CCI_PCLK_RATE(); 	break;
		default: printk("Unknown clock ID [%d] ...\n", id);
		break;
	}

	pr_debug("%s: name %s id %d rate %ld\n",
			__func__, clk_data->name, clk_data->id, rate);
	return rate;
}
					...
					unsigned long *prate)
{
	...
	pr_debug("%s: name %s id %d (%lu, %lu) <%d,%d,%d>\n",
		__func__, clk_data->name, id, drate, rate, pms->P, pms->M, pms->S);
	return rate;
}
...
static struct clk *clk_pll_clock_register(const char *name, const char *parent_name,
				 struct clk_hw *hw, const struct clk_ops *ops, unsigned long flags)
{
	struct clk *clk;
	...
	if (clk_register_clkdev(clk, init.name, NULL))
		pr_err("%s: failed to register lookup for %s", __func__, init.name);
...
static void __init clk_pll_of_clocks_setup(struct device_node *node)
{
	...
	for (i = ID_CPU_FCLK; ID_END > i; i++) {
		...
		clk = clk_pll_clock_register(clk_data->name, parent_name, &clk_data->hw,
					&clk_dev_ops, flags);
		if (clk)
...
static void __init clk_pll_of_clocks_dump(struct device_node *np)
{
	...
	printk("(%d) PLL%d: CPU  FCLK = %10lu, HCLK = %9lu (G1)\n",
		DIV_CPUG1, pll, (ulong)CPU_FCLK_RATE(DIV_CPUG1),
	...
	printk("(%d) PLL%d: MEM  FCLK = %10lu, DCLK = %9lu, BCLK = %9lu, PCLK = %9lu\n",
		...
		clk_data[ID_MEM_BCLK].rate, clk_data[ID_MEM_PCLK].rate);
	...
	printk("(%d) PLL%d: G3D  BCLK = %10lu\n",
		clk_data[ID_G3D_BCLK].div, clk_data[ID_G3D_BCLK].pll,
		clk_data[ID_G3D_BCLK].rate);

	/* coda (vpu) */
	printk("(%d) PLL%d: VPU  BCLK = %10lu, PCLK = %9lu\n",
...
static void __init clk_pll_of_setup(struct device_node *node)
{
	...
	ref_clk_base = ioremap(PHYS_BASE_CLKPWR, SZ_4K);
...
/*
 * NOTE>
 * 	must be located before "sys_clk" node
 *
 * refclk: oscillator {
 * 		compatible = "Nexell,s5p6818,pll";
 * 		ref-freuecny = <27000000>;
 * };
 *
 */
CLK_OF_DECLARE(s5pxx18, "Nexell,s5p6818,refclk", clk_pll_of_setup);
