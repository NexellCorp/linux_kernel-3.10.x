...
static inline long clk_dev_divide(long rate, long request, int align, int *divide)
{
...
static long dev_round_rate(struct clk_hw *hw, unsigned long rate)
{
	...
	step = peri->clk_step;
	mask = peri->in_mask;
	pr_debug("clk: %s reqeust = %ld [input=0x%x]\n", peri->name, rate, mask);
...
next:
	...
	pr_debug("clk: %s, step[%d] src[%d,%d] %ld /(div0: %d * div1: %d) = %ld, %ld diff (%ld)\n",
		peri->name, peri->clk_step, peri->div_src_0, peri->div_src_1, freq_hz,
		peri->div_val_0, peri->div_val_1, new_rate, request, abs(new_rate-request));
...
static int clk_dev_enable(struct clk_hw *hw)
{
	...
	pr_debug("clk: %s enable (BCLK=%s, PCLK=%s)\n",
		peri->name, I_GATE_BCLK & peri->in_mask ? "ON":"PASS",
...
static void __init clk_dev_parse_device_data(struct device_node *np,
						struct clk_dev *clk_data, struct device *dev)
{
...
static void __init clk_dev_of_setup(struct device_node *node)
{
	...
	if (!clk_data) {
		pr_err("Failed to allocate memory for clock generator!\n");
		return;
	}
	...
	for (i = 0; num_clks > i; i++, clk_data++) {
		...
		clk = clk_dev_clock_register(peri[i].name,
					peri[i].parent_name, &clk_data->hw, ops, flags);
		if(NULL == clk)
			continue;
