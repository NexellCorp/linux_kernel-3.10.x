...
static struct s3c24xx_serial_drv_data exynos4210_serial_drv_data = {
	...
	.def_data = &(struct s3c24xx_uart_drv_data) {
...
static inline struct s3c24xx_uart_drv_data *s3c24xx_port_to_data(struct uart_port *port);
...
static int s3c24xx_serial_request_port(struct uart_port *port)
{
	...
	return request_mem_region(port->mapbase, MAP_SIZE, name) != NULL ? 0 : -EBUSY;
}
...
static void s3c24xx_serial_resetport(struct uart_port *port,
				   struct s3c24xx_uart_drv_data *data)
{
	struct s3c24xx_uart_info *info = s3c24xx_port_to_info(port);
				    ...
				    struct platform_device *pdev)
{
	...
	dbg("s3c24xx_serial_init_port: port=%p, pdev=%p\n", port, pdev);
	...
	uport->clk = clk_get(&pdev->dev, NULL);
...
static void s3c24xx_serial_drv_init(void *data, int port)
{
					...
					struct s3c24xx_uart_drv_data **udata, int *port_index)
{
	...
	drv_data = (struct s3c24xx_serial_drv_data *)match->data;
...
{
	struct s3c24xx_uart_port *uport = NULL;
	struct s3c24xx_uart_drv_data *udata = NULL;
	int port_index = pdev->id;
	int ret;

	dbg("s3c24xx_serial_probe dev %s\n", dev_name(&pdev->dev));
	...
	uport->drv_data = s3c24xx_get_driver_data(pdev, &udata, &port_index);
	if (!uport->drv_data) {
...
//			uport->drv_data->def_data;
	...
	INIT_DELAYED_WORK(&uport->resume_work, s3c24xx_resume_work);
 	wake_lock_init(&uport->resume_lock, WAKE_LOCK_SUSPEND,
	             kasprintf(GFP_KERNEL, "UART%u", port_index));
	...
	s3c24xx_serial_ports[port_index] = uport;
